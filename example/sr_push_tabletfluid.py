...
##### NOTE: The recent script is the one using ReFlex made together with Adriaan in the reflex folder!!! ######
...
dark = flex.data.read_stack(path, 'di', sample=sample)
flat = flex.data.read_stack(path, 'io', sample=sample)
proj = flex.data.read_stack(path, 'scan_', sample=sample, skip=sample)
print(np.shape(dark), np.shape(flat), np.shape(proj))


# Parcer for a flexray log file:
geom = flex.data.read_flexraylog(path, sample = sample)

print('\nParsing "scan settings.txt":')
print(geom)

# Write TOML format to disk:
flex.data.write_toml(path + 'metadata.toml', geom)

print('\nReading raw TOML:')
print(flex.data.read_toml(path + 'geometry.toml'))

print('\nParsing raw TOML:')
print(flex.data.read_geometry(path, sample = 4))

meta = flex.data.read_flexraymeta(path, sample=sample)
